...
func deleteOrder(curUser Users) int {
	var prodname string
	fmt.Println("Enter product name to delete")
	fmt.Scanf("%s\n", &prodname)
	for j := 0; j < length; j++ {
...
func userAuth(mobile int64) Users {
...
func main() {
	...
	var curUser Users = userAuth(mobile)
	var op int
	for true {
		fmt.Printf("\n")
		fmt.Println("1 : Get Products")
		fmt.Println("2 : Place Orders")
		fmt.Println("3 : Modify Products")
		fmt.Println("4 : Cancel Products")
		fmt.Println("5 : Print Products")
		fmt.Println("6 : Print Products ordered from me")
		fmt.Println("7 : Login to another account")
		fmt.Scanln(&op)
		if op == 1 {
			showProduct()
		} else if op == 2 {
